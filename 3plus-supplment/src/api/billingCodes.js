...
export async function getBillingCodes(query) {
  ...
  const endpoint = `${base}/BillingCode/GetBillingCode?billingCode=${encodeURIComponent(query || '')}`
  try {
    ...
    if (Array.isArray(data)) {
      ...
      return data.map((it, idx) => ({ name: it.name ?? it.Name ?? String(it), value: it.value ?? it.Value ?? idx }))
    }
